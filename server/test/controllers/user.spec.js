...
describe('USER CONTROLLER', () => {
      ...
      request(app)
        ...
        .end((err, res) => {
          ...
          expect(res.body.message)
            .to.equal('Username taken already. Please use another one.');
          done();
        });
    });
    ...
    it('should not create user with the same email address', (done) => {
      request(app)
        ...
        .end((err, res) => {
          expect(res.status).to.equal(409);
          expect(res.body.message)
            .to.equal('Email taken already. Please use another one.');
          done();
        });
    });
    ...
    it('should require username and password before signin', (done) => {
      request(app)
        ...
        .end((err, res) => {
          expect(res.status).to.equal(400);
          expect(res.body.message)
            .to.equal('Please provide a username and password');
          done();
        });
    });
  ...
  describe('FORGET PASSWORD API -/api/user/forgotpassword', () => {
    it('should throw an error if a valid email is not provided ', (done) => {
      request(app)
    ...
    it(`should send forgot password email if email address exists in the
    database`, (done) => {
        ...
        request(app)
          ...
          .end((err, res) => {
            expect(res.status).to.equal(200);
            expect(res.body.message).to.equal('An email has been sent to ' +
              'user1@gmail.com with further instructions.');
            done();
          });
      });

    it('should not send forgot password email a network error occurs',
      (done) => {
    ...
    it(`should not send forgot password email if email address does not exist
  ...
  describe('RESET PASSWORD API - /api/user/resetpassword/:token', () => {
    it(`should reset user password if password token is associated with a 
    user id`, (done) => {
        transporter.sendMail = () => Promise.resolve(1);
        const resetToken = '0agwAvILWEVS5xDlaTODlIImxZ5NpHBUxzDiwa2kExG7AnzK6G';
        request(app)
    ...
    it('should not reset user password if a network error occurs', (done) => {
      transporter.sendMail = () => Promise.reject(1);
      const resetToken = '2QVwcHW9OyX6SAKsJhXEgemhgqA7qHjaRCmhJ3gf0re8tSBM3X';
      request(app)
        .put(`/api/user/resetpassword/${resetToken}`)
        .set('Accept', 'application/json')
        .send({
    ...
    it(`should not reset user password if password token is not associated with
    a user id`, (done) => {
        request(app)
          ...
          .end((err, res) => {
            expect(res.status).to.equal(400);
            expect(res.body.message)
              .to.equal('Password Reset Token is Invalid or has Expired');
            done();
          });
      });
      ...
      (done) => {
        ...
        request(app)
          ...
          .end((err, res) => {
            expect(res.status).to.equal(400);
            expect(res.body.message)
              .to.equal('Please provide a new password for your account');
            done();
          });
      });
  });
  ...
  describe('SEARCH USER API - /api/search/users', () => {
    ...
    it('should search even if limit and offset not included in query',
      (done) => {
        request(app)
    ...
    it('should not allow unregistered user to search for other users',
      (done) => {
        request(app)
          .get('/api/search/users?searchTerm=a&group=2&limit=1&offset=0')
          .set('Accept', 'application/json')
          .end((err, res) => {
            expect(res.status).to.equal(401);
            expect(res.body.message)
    ...
    it('should not find any user if search query doesn\'t match any user',
      (done) => {
        request(app)
          .get('/api/search/users?searchTerm=blah&group=1&limit=1&offset=0')
          .set('authorization', user1token)
          .end((err, res) => {
            expect(res.status).to.equal(404);
    ...
    it('should not search if group is not specified', (done) => {
      request(app)
        .get('/api/search/users?searchTerm=t&limit=1&offset=0')
        .set('authorization', user1token)
        .end((err, res) => {
          expect(res.status).to.equal(400);
          expect(res.body.message).to.equal('Please provide a valid group ID');
          done();
        });
    });

    it('should not search if group is does not exist', (done) => {
      request(app)
        .get('/api/search/users?searchTerm=t&group=566&limit=1&offset=0')
        .set('authorization', user1token)
        .end((err, res) => {
          expect(res.status).to.equal(404);
  ...
  describe('LIST USER\'S GROUPS API - /api/user/:userId/groups', () => {
    ...
    it('should throw an error if the user ID is not a number', (done) => {
      request(app)
        .get('/api/user/one/groups')
        .set('authorization', user1token)
        .end((err, res) => {
          expect(res.status).to.equal(400);
          expect(res.body.message)
            .to.equal('Please provide a valid user ID');
          done();
        });
    });
